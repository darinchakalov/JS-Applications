import bookSerices from "../services/bookSerices.js";
import { html } from "../utils.js";

let myBooksTemplate = (books) => html` <section id="my-books-page" class="my-books">
	<h1>My Books</h1>
	<!-- Display ul: with list-items for every user's books (if any) -->
    ${books.length > 0 ? html`<ul class="my-books-list">
        ${books.map(singleBookTemlate)}
        </ul>` : noBooksTemplate()}
	

	<!-- Display paragraph: If the user doesn't have his own books  -->
</section>`;

let singleBookTemlate = (book) => html`<li class="otherBooks">
	<h3>${book.title}</h3>
	<p>Type: ${book.type}</p>
	<p class="img"><img src=${book.imageUrl} /></p>
	<a class="button" href="/details/${book._id}">Details</a>
</li>`;
let noBooksTemplate = () => html` <p class="no-books">No books in database!</p>`;

export async function renderMyBooksPage(context) {
    let books = await bookSerices.getUserBooks();
    console.log(books);
	context.renderView(myBooksTemplate(books));
}
